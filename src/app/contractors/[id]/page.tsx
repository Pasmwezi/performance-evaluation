...
export default async function ContractorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!contractor) {
    return <div className="flex items-center justify-center min-h-[50vh] text-slate-500">Contractor not found</div>;
  }

  const totalEvals = contractor.evaluations.length;
  const avgScore = totalEvals > 0 
    ? Math.round(contractor.evaluations.reduce((sum, e) => sum + (e.totalPoints || 0), 0) / totalEvals) 
    : null;
    ...
    <div className="space-y-6">
      ...
      <div className="app-card p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className={`w-14 h-14 rounded-lg flex items-center justify-center text-xl font-bold ${isUnderperforming ? 'bg-rose-500/10 text-rose-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
              {contractor.name[0]}
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-950">{contractor.name}</h1>
              <p className="text-sm text-slate-500">{contractor.address || "No address on file"}</p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            {avgScore !== null && (
      ...
      <div>
        ...
        <div className="space-y-3">
          {contractor.evaluations.map((evaluation) => (
            <div key={evaluation.id} className="app-card p-4">
              ...
              <div className="flex flex-col gap-3 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <Link href={`/contractors/${contractor.id}/evaluations/${evaluation.id}`} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-white/80 px-2.5 py-1.5 font-semibold text-teal-700 transition-colors hover:border-teal-300 hover:bg-teal-50">
          ...
          {contractor.evaluations.length === 0 && (
