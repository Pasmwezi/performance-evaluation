...
export default async function ContractorEvaluationPage({ params }: PageProps<{ id: string; evaluationId: string }>) {
  ...
  let evaluator = null;
  if (evaluation?.createdById) {
    evaluator = await prisma.user.findUnique({
  ...
  let maxPossible = 0;
  const categories = [
    evaluation.qualityOfWorkmanship,
    evaluation.time,
    evaluation.projectManagement,
    evaluation.contractManagement,
    evaluation.healthAndSafety
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="app-card p-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <DetailItem label="Evaluator" value={evaluator ? (evaluator.name ? `${evaluator.name} (${evaluator.email})` : evaluator.email) : "System / Unknown"} />
      ...
      {evaluation.lowScoreJustification && (
        <section className="app-card p-6 border-amber-200 bg-amber-50/20">
          <h2 className="mb-2 text-lg font-semibold text-amber-900 flex items-center gap-2">
            ...
            Low Score Justification Narrative
          </h2>
          <p className="text-sm leading-6 text-slate-700 whitespace-pre-wrap">{evaluation.lowScoreJustification}</p>
        </section>
      )}
    </div>
  );
